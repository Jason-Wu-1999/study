...
当前端请求的数据通过form表单提交时，例如向`/login`发送一个Get请求，获取请求数据的方式如下：

```go
func main() {
   r:=gin.Default()
   r.LoadHTMLFiles("./From/login.html")
   ...
   r.POST("/index", func(c *gin.Context) {
      c.JSON(200,gin.H{
		//name := c.DefaultPostForm("username", "小王子")
...
<body>
    <form action="/index" method="post" novalidate autocapitalize="off">
        <div>
...
==首先：向/login发送Get请求，请求之后访问到login.html文件==

==当点击登录时，<form action="/index" method="post"，会向/index发送post请求==
...
func main() {
	...
	r.GET("/bolg/:year/:month" , func(c *gin.Context) {
		year:=c.Param("year")
		month:=c.Param("month")//返回字符串类型
		c.JSON(200,gin.H{
			"year":year,
			"month":month,
		})
